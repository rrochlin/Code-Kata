'''
The Vigenère cipher is a classic cipher originally developed by Italian cryptographer Giovan 
Battista Bellaso and published in 1553. It is named after a later French cryptographer Blaise 
de Vigenère, who had developed a stronger autokey cipher (a cipher that incorporates the message 
of the text into the key).

The cipher is easy to understand and implement, but survived three centuries of attempts to break 
it, earning it the nickname "le chiffre indéchiffrable" or "the indecipherable cipher."
...
in a Caesar cipher of shift 3, A would become D, B would become E, Y would become B and so on. The 
...
implementations repeat the key over characters only if they are part of the alphabet -- this is not 
the case here.

The shift is derived by applying a Caesar shift to a character with the corresponding index of the key 
in the alphabet.
    ...
    def encode(self, text):
        ...
        for char in text:
            if char in self.alphabet:
                start = self.alphabet.index(char)
                shift = self.alphabet.index(self.key[key_index])
                end = self.alphabet[(start + shift) % len(self.alphabet)]
                encoded += end
            else:
                encoded += char
